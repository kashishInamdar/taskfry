...
export const UserContextProvider = ({ children }) => {
  ...
  const userLoginStatus = async () => {
    let loggedIn = false;
    try {
      const res = await axios.get(`http://localhost:8000/api/v1/login-status`, {
        withCredentials: true, // send cookies to the server
      });
  ...
  const updateUser = async (e, data) => {
    ...
    try {
      const res = await axios.patch(`http://localhost:8000/api/v1/user`, data, {
        withCredentials: true, // send cookies to the server
      });
  ...
  const verifyUser = async (token) => {
    ...
    try {
      const res = await axios.post(
        `http://localhost:8000/api/v1/verify-user/${token}`,
        {},
        {
          withCredentials: true, // send cookies to the server
        }
